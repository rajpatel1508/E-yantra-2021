...
def get_vision_sensor_image(client_id):
	...
	##############	ADD YOUR CODE HERE	##############
	res,sensorHandle=sim.simxGetObjectHandle(client_id,'vision_sensor',sim.simx_opmode_blocking)
...
def detect_qr_codes(transformed_image):
	...
	for code in codes:
		temp=[]
		temp.append(code.data.decode("ascii"))
		A,B,C,D = code.polygon
		x = (A[0] + B[0] + C[0] + D[0])/4
		y = (A[1] + B[1] + C[1] + D[1])/4
		temp2=[x,y]
		temp.append(tuple(temp2))
		qr_codes.append(temp)

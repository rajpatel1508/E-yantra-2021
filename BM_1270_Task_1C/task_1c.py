...
def read_distance_sensor(client_id, sensor_handle):
	...
	##############	ADD YOUR CODE HERE	##############
	return_Code,detected,detected_Point,detectedObjectHandle,detectedNormal=sim.simxReadProximitySensor(client_id,sensor_handle,sim.simx_opmode_blocking)
	if detected:
...
def control_logic(client_id):
	...
	##############  ADD YOUR CODE HERE  ##############
	return_Code,left_handle=sim.simxGetObjectHandle(client_id,'left_joint',sim.simx_opmode_blocking)
	return_Code,right_handle=sim.simxGetObjectHandle(client_id,'right_joint',sim.simx_opmode_blocking)
	return_Code,d1_handle=sim.simxGetObjectHandle(client_id,'distance_sensor_1',sim.simx_opmode_blocking)
	return_Code,bot=sim.simxGetObjectHandle(client_id,'Diff_Drive_Bot',sim.simx_opmode_blocking)
	x=-1
	l=[1.55,3.127,-1.57]
	while x<3:
		det,dis = read_distance_sensor(client_id,d1_handle)
		if det and dis <= 0.15:
			rotSpeed=0.4
			return_code,Angle=sim.simxGetObjectOrientation(client_id,bot,-1,sim.simx_opmode_blocking)
			Angle = round(Angle[2],3)
			x=x+1
			if(x==3):
				break
			while Angle < l[x]:
				return_code,Angle=sim.simxGetObjectOrientation(client_id,bot,-1,sim.simx_opmode_blocking)
				Angle = round(Angle[2],3)
				sim.simxSetJointTargetVelocity(client_id,left_handle,-rotSpeed,sim.simx_opmode_streaming)
				sim.simxSetJointTargetVelocity(client_id,right_handle,rotSpeed,sim.simx_opmode_streaming)
			sim.simxSetJointTargetVelocity(client_id,left_handle,1,sim.simx_opmode_streaming)
			sim.simxSetJointTargetVelocity(client_id,right_handle,1,sim.simx_opmode_streaming)
	sim.simxSetJointTargetVelocity(client_id,left_handle,0,sim.simx_opmode_streaming)
	sim.simxSetJointTargetVelocity(client_id,right_handle,0,sim.simx_opmode_streaming)

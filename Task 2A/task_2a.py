...
# Team ID:			[ 1270 ]
# Author List:		[ Soham Das, Satvik Raj Patel, Ayush Kumar, Utsav Anand ]
# Filename:			task_2a.py
# Functions:		
# Global variables:	
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	transformed_depth_image = np.reshape(transformed_depth_image,(image_resolution[0],image_resolution[1]))
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	img2=cv2.cvtColor(transformed_image,cv2.COLOR_RGB2GRAY)
	r,threshold=cv2.threshold(img2,0,255,cv2.THRESH_BINARY)
	cont,hierarchy=cv2.findContours(threshold,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
	for i in range(0,len(cont)):
		...
		if (transformed_image[Y,X,0]>128 and transformed_image[Y,X,0]<=255 and transformed_image[Y,X,1]<=255 and transformed_image[Y,X,1]>128 and transformed_image[Y,X,2]<127):
			berries_dictionary["Lemon"].append(tuple([X,Y,transformed_depth_image[Y,X]]))
		elif(transformed_image[Y,X,2]<=255 and transformed_image[Y,X,2]>128 and transformed_image[Y,X,1]<127 and transformed_image[Y,X,0]<127):
			berries_dictionary["Blueberry"].append(tuple([X,Y,transformed_depth_image[Y,X]]))
		elif(transformed_image[Y,X,2]<127 and transformed_image[Y,X,1]<127 and transformed_image[Y,X,0]<=255 and transformed_image[Y,X,0]>128):
			berries_dictionary["Strawberry"].append(tuple([X,Y,transformed_depth_image[Y,X]]))
...
def detect_berry_positions(berries_dictionary):
	...
	##############	ADD YOUR CODE HERE	##############
	berry_positions_dictionary = {berries[0]:[], berries[1]:[], berries[2]:[]}
	for i in range(0,3):
		if not berries_dictionary[berries[i]]:
			continue
		else:
			for j in range(0,len(berries_dictionary[berries[i]])):
				x = berries_dictionary[berries[i]][j][0]
				y = berries_dictionary[berries[i]][j][1]
				z = berries_dictionary[berries[i]][j][2]
				x = (x - 256)/512
				y = (y - 256)/512
				d = 0.01 + z*1.99 + 0.015
				berry_positions_dictionary[berries[i]].append(tuple([x,y,d]))
